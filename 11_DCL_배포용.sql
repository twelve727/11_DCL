...
CREATE USER cjo_sample IDENTIFIED BY sample1234;

-- 2. 새 연결(접속 방법) 추가
-- CREATE_SESSION 권한 X 에러 = 접속권한을 갖고 있지 않음

-- 3. 접속 권한 부여
-- [작성법.]
-- GRANT 권한, 권한, ... TO 사용자명
GRANT CREATE SESSION TO cjo_sample;

-- 4. 다시 연결(접속 방법 추가) -> 성공

-- 5. (sample) 계정에서 테이블 생성해보기
...
--> CREATE TABLE 테이블 생성 권한
-- + TABLESPACE (데이터를 저장할 수 있는 공간) 할당

-- 6. (SYS) 테이블 생성 권한 + TABLESPACE 할당
GRANT CREATE TABLE TO cjo_sample;

ALTER USER cjo_sample DEFAULT TABLESPACE
...
--> 해당 계정은 지정된 권한을 이용하여 특정 권한을 갖게됨

-- (SYS) sample 계정에 SYS의 CONNECT, RESOURCE 부여해보기
GRANT CONNECT,RESOURCE TO cjo_sample;
...
-- KH / cjo_sample 사용자 계정끼리 서로 객체 접근 권한 부여
...
-- 2. (kh)에서 cjo_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO cjo_sample;

-- 3. (sample) 에서 다시 kh.EMPLOYEE 조회

SELECT * FROM kh.EMPLOYEE; -- 됨
SELECT * FROM kh.DEPARTMENT; -- 권한 없어서 안됨
...
-- 5. (kh) sample 계정에 부여한 employe 권한 회수
-- REVOKE 사용

-- [ 권한 회수 작성법 ]
-- REVOKE 객체권한 ON 객체명 FROM 사용자명;

REVOKE SELECT ON EMPLOYEE FROM cjo_SAMPLE
