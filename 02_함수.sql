...
-- EXTRACT(DAY FROM 날짜) : 일 만 추출

-- EMPLOYEE 테이블에서
-- 각 사원의 이름, 입사년도, 월, 일을 조회하시오
...
/* 형변환 함수 */
-- 문자열(CHAR), 숫자(NUMBER), 날짜(DATE) 끼리 형 변환이 가능

/* 문자열로 변환 */
-- TO_CHAR(날짜, [포맷]) : 날짜형 데이터를 문자형 데이터로 변경 ( 앞에껄 뒤로 )
-- TO_CHAR(숫자, [포맷]) : 날짜형 데이터를 숫자형 데이터로 변경 ( 앞에껄 뒤로 )

-- < 숫자 변환 시 포맷 패턴 >
-- 9 : 숫자 한 칸을 의미, 여러개 작성시 <오른쪽> 정렬
-- 0 : 숫자 한 칸을 의미, 여러개 작성시 <오른쪽> 정렬 + 빈 칸에 0을 추가
...
-- AM / PM : 오전/오후 표시
-- HH : 시간 / HH24 : 24시간 표기법
-- MI : 분 (MM이 아니니 주의!) / SS : 초
-- DAY : 요일(전체) / DY : 요일(요일명만 표시)

SELECT SYSDATE FROM DUAL; // 현재 년월일날짜시간 표시 // 2023-01-10 10:21:18.000

-- 2023-01-10 10:21:18.000 화요일
SELECT TO_CHAR(SYSDATE, 'YYYY/MM/DD HH24:MI:SS:DAY') FROM DUAL; 

-- 2023년 01월 10일 (화)
SELECT TO_CHAR(SYSDATE, 'YYYY "년" MM "월" DD "일" (DY)' ) FROM DUAL;
-- 년,월,일이 날짜를 나타내는 패턴으로 인식이 안되서 오류가 발생
...
SELECT TO_DATE('510505' , 'YYMMDD') FROM DUAL; -- 2051-05-05 : YY 미래 현재
SELECT TO_DATE('510505' , 'RRMMDD') FROM DUAL; -- 1951-05-05 : RR 과거

-- EMPLOYEE 테이블에서 각 직원 태어난 생년월일(1990년 05월 13일) 조회 (별칭 생년월일)
-- 621231-19853634

SELECT TO_DATE()  AS 생년월일
FROM EMPLOYEE 
...
-- NVL(컬럼값, 컬럼값이 NULL 일 때 바꿀 값 ) : NULL인 컬럼값을 다른 값으로 변경
/* NULL과 산술 연산을 진행하면 결과는 < DB에서는 무조건 NULL > */
...
SELECT EMP_NAME, NVL2(BONUS, '0', 'X') FROM EMPLOYEE;
...
-- AVG(숫자가 기록된 컬럼명) : 평균
-- 전 직원 급여 평균을 구하시오
SELECT ROUND( AVG(SALARY) ) FROM EMPLOYEE; - 30,147,663
 
-- 부서 코드가 'D9'인 사원들의 급여 합, 평균을 구하시오
SELECT SUM(SALARY), ROUND(AVG(SALARY))
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';

-- 해석순서 : FROM-WHERE-SELECT 순서라,  D9 -> EMPLOYEE D9 SUM,AVG() 
...
-- 급여 최대값 가장 빠른 입사일, 알파벳 순서가 가장 느린 이메일
