...
SELECT CEIL(AVG(SALARY)) -- CEIL 올림 , AVG 평균
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY >= (SELECT CEIL(AVG(SALARY)) -- CEIL 올림 , AVG 평균
				FROM EMPLOYEE);
...
SELECT MIN(SALARY) FROM EMPLOYEE; -- 단일행이 맞는지 꺼내서 실행해보기
...
SELECT EMP_NAME, JOB_CODE, DEPT_TITLE, HIRE_DATE
FROM EMPLOYEE
WHERE DEPT_CODE = ()
AND JOB_CODE = ()
...
FROM EMPLOYEE
WHERE ENT_YN = 'Y' -- Y/N중 Y, 퇴직한사람
AND SUBSTR(EMP_NO, 8, 1) = '2'; -- 여직원
-- D8, J6

-- 2) 퇴사한 여직원과 같은 부서, 같은 직급 (다중 열 서브쿼리)
SELECT EMP_NAME, JOB_CODE, DEPT_NAME, HIRE_DATE
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 1, 2) = '77' --77년생 
AND SUBSTR(EMP_NO, 8,1) = '2'); -- 여자사원
--(이후 DEPTCODE,MANAGERID 부서랑, 사수 구하기)
...
-- 1) 급여를 200만, 600만 둘 다 받는 직원 (200만, 600만이 평균급여라 생각 할 경우)
...
FROM EMPLOYEE
GROUP BY JOB_CODE; -- 결과값 2624373.3333 지우기 -> TRUNC 사용

-- 왜 TRUNC에서 -4 사용하는지 확인할것!!
...
-- EMPLOYEE 23개의 행이 있다면, 상호연관으로 23개의 서브쿼리가 생긴다는 의미

-- 위에선 서브쿼리 결과를 정해놓고 사용했다면, 상호연관 서브쿼리는 메인결과가 중요

-- 상관쿼리는 먼저 메인쿼리를 한 행 조회하고
-- 해당 행이 서브쿼리의 조건을 충족하는지 확인한 후
-- SELECT를 진행함
...
WHERE SALARY > (SELECT AVG(SALARY)
			...
			--EMP뒤에 MAIN , EMP뒤에 SUB,
			--SUB문 WHERE에 SUB.JOB_CODE  = MAIN.JOB_CODE 

			-- J1 = 800, 초과여서 선동일 자름
			-- 선동일 짜르고, J2 다시 찾음, J3 다시찾음 . . .
			-- 마지막 최종으로 나오는 RESULT SET을 찾아줌
			
			-- MAIN -> SUB -> MAIN
			-- 이래서 MAIN이 SUB에 영향을 많이 끼침
...
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE) -- JOIN 사용
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
WHERE ENT_YN = 'N' -- 아니오
AND HIRE_DATE = (SELECT MIN(HIRE_DATE) -- HIRE_DATE이기 때문에 MIN 사용
				 FROM EMPLOYEE SUB
				 WHERE SUB.DEPT_CODE = MAIN.DEPT_CODE) 
				 -- D1~D9 이라는 DEPT_CODE에서 가장 빠른 입사일
				 -- 2007-03-20 00:00:00.000
ORDER BY HIRE_DATE; -- ORDER BY로 정렬해주기

-- 1) MAIN의 1행 DEPT_CODE를 SUB에 대입 
-- 2) SUB를 수행
...
-- 6. 스칼라 서브쿼리
-- SELECT 절에 사용되는 서브쿼리 결과로 [1행만 반환]한다.
-- SQL에서 단일 값을 가르켜 [스칼라] 라고 한다.
-- SELECT절에 작성되는 단일행 서브쿼리


-- <예제>
-- 모든 직원의 이름, 직급, 급여, 전체사원 중 가장 높은 급여 [와의 차?]

SELECT EMP_NAME, JOB_CODE, SALARY,
(SELECT MAX(SALARY) FROM EMPLOYEE) - SALARY
--전체사원 중 가장 높은 급여 [와의 차? = 단순히 -SALARY]
FROM EMPLOYEE;
...
((SELECT EMP_NAME
FROM EMPLOYEE SUB
WHERE SUB.EMP_ID = MAIN.MANGER_ID),'없음')
FROM EMPLOYEE MAIN;*/
...
WHERE 부서 = '기술지원부';
-- 부서라는 타이틀을 SELECT에서 지정해주었기 때문에
-- WHERE에서도 WHERE 부서 <-- 만 가능
...
-- 08. RANK() OVER / DENSE_RANK() OVER
-- 동일한 순위 이후의 등수를, 동일한 인원 수 많큼 건너뛰고 순위계산
-- EX) 공동 1위가 2명[이면], 다음 순위는 [2위가 아닌 3위]
...
FROM (SELECT EMP_NAME, SALARY
		FROM EMPLOYEE
		ORDER BY SALARY DESC); -- 전형돈,윤은혜 19위 20위? RANKOVER
		
-- 2) RANK()OVER 정렬 순서
SELECT RANK() OVER(ORDER BY SALARY DESC) 순위,
	EMP_NAME, SALARY
	FROM EMPLOYEE; -- 전형돈,윤은혜 동일하게 19위, 이후 21위 박나라

	
	
-- DENSE_RANK() OVER : 동일한 순위 이후의 등수를 이후의 순위로 계산
-- EX) 공동 1위가 2명[이어도], 다음 순위는 [2위]

SELECT DENSE_RANK() OVER(ORDER BY SALARY DESC) 순위,
	EMP_NAME, SALARY
	FROM EMPLOYEE; -- 박나라 21위가 아닌 20위로 지정

	
-- @@ RANK() OVER / DENSE_RANK() OVER 시험에나옴 @@
-- @@ RANK() OVER / DENSE_RANK() OVER 시험에나옴 @@
-- @@ RANK() OVER / DENSE_RANK() OVER 시험에나옴 @@
...
SELECT ROWNUM, EMP_NAME, SALARY
FROM TOP_SAL -- ROWNUM 다 표기
WHERE ROWNUM <= 10; -- ROWNUM 10위까지 표기
