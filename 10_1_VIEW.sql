...
-- SQL Error [1031] [42000]: ORA-01031: (VIEW 생성)권한이 불충분합니다
-- KH란 계정은 VIEW를 만들 권한이 없다.
...
CREATE OR REPLACE VIEW V_DCOPT2
AS SELECT DEPT_ID, LOCATION_ID FROM DEPT_COPY2;

-- 뷰 생성 확인
SELECT * FROM V_DCOPT2; --컬럼 하나 빠진것 확인!

-- 뷰를 이용한 INSERT
INSERT INTO V_DCOPT2 VALUES('D0','L3');


-- 삽입 확인
SELECT * FROM V_DCOPT2 --> VIEW에 'D0'가 삽입된 걸 확인함
--> 가상의 테이블인 VIEW에 데이터 삽입이 된 것이 아닙니다!

-- 원본 테이블 확인
SELECT * FROM DEPT_COPY2;
--> VIEW에 삽입한 내용이 원본 테이블에 존재함
--> VIEW를 이용한 DML 구문이 원본에 영향을 미친다. (DO,NULL,L3)

-- VIEW를 이용한 DML시 발생하는 문제점 == 제약조건 위배현상
...
-- WITH READ ONLY 사용해서 에러가 발생한 것임
